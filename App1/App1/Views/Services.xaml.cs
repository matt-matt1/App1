...
namespace App1.Views
{
	...
	{
		public string feedUrl = "http://yumatechnical.com/en/module/ps_feeder/rss?id_category=15&orderby=position&orderway=asc";
		...
		public async void PutFeedContents() // load and parse RSS feed
		{
			...
			if (true||await DisplayAlert((Settings.Internet != "") ? Settings.Internet : "Internet",
				...
				(Settings.No != "") ? Settings.No : "No"))
			{
				...
				if (CrossConnectivity.Current.IsConnected)
				{
					try
					{
						...
						App.DB.DropTable<ServicesTbl>();
						string result = "";
						try
						{
							result = App.DB.CreateTable<ServicesTbl>().ToString();
						}
						...
						List<ServicesTbl> DBentries = new List<ServicesTbl>();
